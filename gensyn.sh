...
if [ -f "$SWARM_DIR/swarm.pem" ]; then
    if [ "$SWARM_CHOICE" == "1" ]; then
        ...
        cd $HOME && git clone https://github.com/cloudservices-devops/rl-swarm.git > /dev/null 2>&1
    ...
    cd $HOME && [ -d rl-swarm ] && rm -rf rl-swarm; git clone https://github.com/cloudservices-devops/rl-swarm.git > /dev/null 2>&1
fi
